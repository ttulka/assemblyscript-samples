...
export function mandelbrot_big(
  ...
  const offX = Big.of(offsetX).times(ratioX).plus(X1);
  const offY = Big.of(offsetY).times(ratioY).plus(Y1);
...
export function mandelbrot_native(
  ...
  const offX = offsetX * ratioX + X1;
  const offY = offsetY * ratioY + Y1;
